...
{
    NUMBER_OF_NODES = par("n").intValue();
    ACK_TIMEOUT_DURATION = par("timeout").doubleValue();
    TIMEOUT_DURATION = par("ack_timeout").doubleValue();
...
void Node::handleMessage(cMessage *msg)
{
    ...
    if(type_received == frame_from_network || type_received == network_enable)
    {
        ...
        if(type_received == frame_from_network){
            ...
            EV<<"Sending handshake from "<<getIndex()<<" to "<<rand<<"\n";
            send(mmsg, "out");
    ...
    else if(type_received == data)
    {
        ...
        EV << "Data received at " << node_number << " from " << frame_received->getSource_node() << " with sequence of "<< frame_received->getSequence() <<"\n";

        //Hamming detection and correction at receiver
        ...
        mypayload.erase(0,8);
        std::string originalpayload = mypayload;
        ...
        while(r != 0){
            ...
            originalpayload.erase(i-j-1,1);
            i = i*2;
            j++;
            r--;
        }
        originalpayload = convertToChar(originalpayload);
        mypayload = convertToChar(mypayload);
        frame_received->setM_Payload(mypayload.c_str());
        EV<<"Message received: "<<originalpayload<<"\n";
        if(errorPosition != 0){
            EV<<"Message after correction: "<<frame_received->getM_Payload()<<"\n";
            if(mypayload == originalpayload){
                EV<<"Error was in a hamming check bit\n";
            }
        }
...
void Node::send_frame(frame_type type, sequence_number frame_no, sequence_number ack, unsigned int destination)
{
    ...
    rand=uniform(0,1)*10;
    //generating lost frames
    if(rand >= noise)
    {
        rand=uniform(0,1)*10;
        //generating delayed frames
        if(rand < noise)
        {
...
std::string Node::convertToChar(std::string message) {
    ...
    for (int i = 0; i < message.length(); i+=8) {
        std::string character = message.substr(i, 8);
        sentence += static_cast<char>(std::bitset<8>(character).to_ulong());
    }
    return sentence;
}
